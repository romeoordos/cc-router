...
if (helpFlag) {
  console.log(`
CC Router - Intelligent API router for Claude Code
...
Configuration:
  The router reads router_config.jsonc from the current working directory.
  See README.md for configuration options.

Examples:
  cc-router                    # Start with TUI on default port
  cc-router --port 3010        # Start on port 3010
  cc-router --no-tui           # Start without TUI
  PORT=3010 cc-router          # Use PORT env variable
`);
  process.exit(0);
}
...
if (versionFlag) {
  ...
  console.log(`cc-router v${pkg.version}`);
  process.exit(0);
}

// Determine port (CLI arg > env var > default)
let PORT = Number(Bun.env.PORT) || 3010;
if (portArg) {
  const portValue = portArg.split("=")[1];
  PORT = Number(portValue) || PORT;
}
...
const TUI_ENABLED = process.stdout.isTTY && !noTuiFlag;

// ANSI colors for console output (only when TUI is disabled)
const colors = {
  reset: "\x1b[0m",
  bright: "\x1b[1m",
  dim: "\x1b[2m",
...
async function logRequest(request: Request): Promise<void> {
  ...
  console.log(
    `${colors.bright}${colors.magenta}REQUEST${colors.reset} ${colors.gray}${timestamp}${colors.reset}`
  );
  console.log(colors.dim + "─".repeat(60) + colors.reset);

  console.log(
    `${colors.green}${request.method}${colors.reset} ${colors.bright}${url.pathname}${colors.reset}`
  );
...
if (!TUI_ENABLED) {
  ...
  console.log(`${colors.green}Starting CC Router...${colors.reset}`);
}
...
const server = Bun.serve({
  ...
  async fetch(request: Request) {
    ...
    const knownRoutes = ["/v1/messages", "/v1/messages/count_tokens", "/health"];
    const isKnownRoute = knownRoutes.includes(pathname) &&
      (pathname === "/health" ? method === "GET" : method === "POST");

    if (!TUI_ENABLED) {
      if (isKnownRoute) {
        console.log(
          `${colors.gray}${new Date().toISOString()}${colors.reset} ${colors.green}${method}${colors.reset} ${pathname}`
        );
      } else {
